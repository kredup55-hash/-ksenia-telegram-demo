...
# Эталонные фразы — пунктуация управляет интонацией
PITCH = {
    "agree_intro": "ну... смотрите. по машинам сейчас так...",
    "cars": "чери тиго семь, — это две тыщи в начале, а потом две двести. вот. джили атлас — тоже две двести.",
    "perks": "своя мойка... деньги выводишь когда хочешь... залогов нет, первый день бесплатно.",
    "question": "слушайте, может попробуете... на этой неделе?",
}
...
ПРАВИЛА ЖИВОГО ЗВУЧАНИЯ:
1. Многоточие (...) — вдох, пауза, раздумье
2. Тире (—) перед ценой — интонационный акцент на цифре
3. Бренды пиши просто: "чери тиго семь," и "джили атлас" (без заглавных внутри)
4. Начинай с низкой ноты: "ну... смотрите.", "вот...", "слушайте..."
5. Вопрос — всегда короткий хвост после паузы: "может попробуете... на этой неделе?" или "интересно будет, как думаете?"

РЕАГИРУЙ на слова клиента перед ответом: "угу...", "ага...", "понял...", "ого...", "ничего себе..."
...
ВОПРОС В КОНЦЕ — всегда короткий: "{PITCH['question']}"
"""
...
def post_process_text(text: str) -> str:
    """Финальный фильтр — только исправления без капслока"""
    text = remove_emoji(text)
    fixes = [
        # Убираем капслок если Claude всё же добавит
        (r'тЫщи', 'тыщи'), (r'тЫщу', 'тыщу'),
        (r'двЕсти', 'двести'), (r'чЕри', 'чери'), (r'тИго', 'тиго'),
        (r'сЕм\b', 'семь,'), (r'сЕмь', 'семь,'),  # капслок → правильное написание
...
async def tts(text: str) -> bytes:
    ...
    payload = {
        ...
        "voice_settings": {
            "stability": 0.38,        # чуть разболтанный — живее
            "similarity_boost": 0.75,
            "style": 0.40,            # выразительность для вопросов
