...
SYSTEM_PROMPT = """Ты Ксения из таксопарка Моментум. Профессиональный, но дружелюбный менеджер. ТЫ УЖЕ ПОЗДОРОВАЛАСЬ.
...
ПИШИ БРЕНДЫ И ЦЕНЫ СТРОГО ТАК:
- машина: "чери тиго сём" (одна г, в конце ё)
- машина: "джили атлас"
- 2000 руб: "две тыщи"
- 2200 руб: "две двести"
...
- тире (—) перед каждой ценой — это пауза и акцент
- не больше 3 предложений подряд
- вопрос в конце — с мягким вводным словом: "как считаете, интересно попробовать?" или "что скажете, есть смысл обсудить?"

КОГДА КЛИЕНТ СОГЛАСЕН СЛУШАТЬ:
"отлично. смотрите, по машинам сейчас так. чери тиго сём — две тыщи первые две недели, потом две двести. джили атлас — две двести. первый день бесплатный, залогов нет. как считаете, интересно попробовать?"
"""

def post_process_text(text: str) -> str:
    """Фонетический фильтр — исправляет текст перед отправкой в ElevenLabs"""
    fixes = [
        # Бренды
        (r'черри\s+тигго', 'чери тиго'),
        (r'чери\s+тигго', 'чери тиго'),
        (r'чери\s+тиго\s+семь', 'чери тиго сём'),
        (r'чери\s+тиго\s+7', 'чери тиго сём'),
        (r'чери\s+тиго\s+про', 'чери тиго сём'),
        # Цены — заменяем официальные формы на разговорные
        ...
        (r'\bтысячи\b', 'тыщи'),
        (r'\bтысяча\b', 'тыща'),
        # Вопросы — убираем "а?" в конце
        (r',?\s*а\?$', '?'),
        (r',?\s*да\?$', '?'),
        # Пауза перед финальным вопросом
        (r'([^.!?])\s+(как считаете)', r'\1. \2'),
        (r'([^.!?])\s+(что скажете)', r'\1. \2'),
        (r'([^.!?])\s+(интересно попробовать)', r'\1. \2'),
    ]
