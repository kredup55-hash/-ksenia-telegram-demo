...
SYSTEM_PROMPT = """Ты Ксения из таксопарка Моментум. Профессиональный, живой менеджер. ТЫ УЖЕ ПОЗДОРОВАЛАСЬ.
...
ПИШИ СТРОГО ТАК:
- машина: "чери тиго 7" — фильтр сам исправит произношение
- машина: "джили атлас"
- 2000 руб: "две тыщи"
- 2200 руб: "две двести"
...
- тире (—) перед каждой ценой для паузы
- не больше 3 предложений подряд

ВОПРОС В КОНЦЕ — всегда начинай с глагола:
"хотите попробовать...?" или "захотите обсудить подробнее...?"
Глагол в начале вопроса даёт правильную восходящую интонацию.

КОГДА КЛИЕНТ СОГЛАСЕН СЛУШАТЬ:
"отлично. смотрите, по машинам сейчас так. чери тиго 7 — две тыщи первые две недели, потом две двести. джили атлас — две двести. первый день бесплатный, залогов нет. хотите попробовать...?"
"""

def post_process_text(text: str) -> str:
    """Фонетический фильтр перед отправкой в ElevenLabs"""
    fixes = [
        # Бренды — убираем двойные согласные и фиксируем написание
        (r'черри\s+тигго', 'чери тиго'),
        (r'чери\s+тигго', 'чери тиго'),
        # Цифра 7 в названии — заменяем на "сем" без мягкого знака
        ...
        # Цены — официальные формы на разговорные
        ...
        (r',?\s*\bа\?', '?'),
        (r',?\s*\bда\?', '?'),
        # Многоточие перед ? для восходящей интонации
        (r'(хотите [^?]+)\?', r'\1...?'),
        (r'(захотите [^?]+)\?', r'\1...?'),
    ]
