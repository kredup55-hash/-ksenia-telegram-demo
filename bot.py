...
SYSTEM_PROMPT = """Ты Ксения из таксопарка Моментум. Живая девушка-менеджер, говоришь немного небрежно с паузами. ТЫ УЖЕ ПОЗДОРОВАЛАСЬ.

НЕЛЬЗЯ:
- здороваться снова
- говорить "да конечно", "хорошо", "приветик"
- писать заглавными буквами или латиницей

ПИШИ СТРОГО ТАК (фильтр исправит произношение автоматически):
- машина: "чери тиго семь"
- машина: "джили атлас"
- 2000 руб: "две тыщи"
- 2200 руб: "две двести"
...
- используй ".." между смысловыми блоками для пауз-вдохов
- не больше 3 предложений подряд
- тире (—) перед ценой

ВОПРОС В КОНЦЕ — всегда начинай с "слушайте, а...":
"слушайте, а интересно было бы попробовать на этой неделе?"
Частица "а" после запятой поднимает интонацию вверх.

КОГДА КЛИЕНТ СОГЛАСЕН СЛУШАТЬ — скажи примерно так:
"отлично.. смотрите.. по машинам сейчас так. чери тиго семь — две тыщи первые две недели, потом две двести. джили атлас — тоже две двести. первый день бесплатный, залогов нет. слушайте, а интересно было бы попробовать на этой неделе?"
"""

def post_process_text(text: str) -> str:
    """Фонетический фильтр — исправляет текст перед ElevenLabs"""
    fixes = [
        # Убираем капслок если Claude вдруг добавит
        # Бренды
        (r'черри\s+тигго', 'чери тиго'),
        (r'чери\s+тигго', 'чери тиго'),
        # "семь" в названии машины — убираем мягкий знак чтобы не было "сём"
        (r'чери\s+тиго\s+семь', 'чери.. тиго.. сем'),
        (r'чери\s+тиго\s+сём', 'чери.. тиго.. сем'),
        (r'чери\s+тиго\s+7', 'чери.. тиго.. сем'),
        (r'чери\s+тиго\s+про', 'чери.. тиго.. сем'),
        # Цены
        ...
        (r'\bтысячи\b', 'тыщи'),
        (r'\bтысяча\b', 'тыща'),
        # Убираем "а?" и "да?" в конце
        (r',?\s*\bа\?\s*$', '?'),
        (r',?\s*\bда\?\s*$', '?'),
        # Убираем тройные и более многоточия — оставляем двойные
        (r'\.{3,}', '..'),
        # Капслок → строчные (на случай если Claude нарушит правило)
        # Обрабатываем отдельные слова с заглавными в середине
        (r'тЫщи', 'тыщи'),
        (r'тЫщу', 'тыщу'),
        (r'двЕсти', 'двести'),
        (r'чЕри', 'чери'),
    ...
    # Убираем случайный капслок в словах (ТиГГо → тиго и т.д.)
    # Оставляем только строчные
    def fix_caps(m):
        word = m.group(0)
        # Если слово целиком заглавное — это аббревиатура, не трогаем
