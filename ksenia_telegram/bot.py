...
ВАЖНО ПРО ЦЕНЫ — произноси медленно и чётко:
Когда называешь цену — замедляйся, делай запятую перед суммой.
Пиши так: "стоит, тысяча семьсот девяносто рублей в день"
НЕ пиши цифры слитно — только через запятые для пауз.
Цифры ТОЛЬКО СЛОВАМИ: "две пятьсот", "тысяча семьсот девяносто", "три тысячи".
...
2. Водитель говорит "да" — спроси одну причину ухода
3. Присоединись: "Да, понятно" / "Без проблем" / "Смотрите..."
4. Быстро перечисли выгоды, цену называй медленно и чётко
5. Закрытие: "Записываю вас, когда удобно подъехать?"

РЕАЛЬНЫЕ ЦЕНЫ (всегда словами, медленно):
Комфорт+: Tiggo 7 Pro — новым, две тысячи в день, две недели, затем две четыреста.
Atlas Pro — новым, две двести, затем две восемьсот.
Belgee X70 — новым, две пятьсот, затем две восемьсот.
Комфорт: Tiggo 4 Pro — новым, тысяча семьсот девяносто в день, две недели, затем две двести.
Coolray — новым, две тысячи, затем две триста.
Без залога: Tenet T7 и Arrizo 8 — две пятьсот в день, бонус тринадцать пятьсот за активную работу.
...
СКРИПТЫ ВОЗРАЖЕНИЙ:
Дорого: "Смотрите, залогов нет, первый день бесплатно, ОСАГО и ТО за наш счёт. Плюс бонус тринадцать пятьсот при активной работе."
Другой парк: "Отлично! Переходите — три дня аренды бесплатно. Главное взять минимум на две недели."
Мало заказов: "На новом авто приоритет в Яндексе автоматически выше — это напрямую влияет на заказы."
...
ГОВОРИ НА ВЫ. Максимум 3 предложения. Заканчивай вопросом или конкретным действием."""


def add_silence_padding(mp3_bytes: bytes, silence_ms: int = 150) -> bytes:
    """Добавляет тишину в конец аудио чтобы не обрезались последние буквы"""
    try:
    ...
    try:
        async with aiohttp.ClientSession() as s:
            async with s.post(url, json=payload, headers=headers) as r:
                if r.status == 200:
                    raw = await r.read()
                    # Добавляем 150мс тишины в конец — предотвращает обрезание последних букв
                    return add_silence_padding(raw, silence_ms=150)
                err = await r.text()
