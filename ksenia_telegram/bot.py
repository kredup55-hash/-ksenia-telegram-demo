...
СТРУКТУРА:
1. Представилась + подскажите вы раньше работали + условия обновились + уделите пару минут?
2. "да" → слушайте, а почему тогда ушли?
3. Присоединись + решение под ситуацию
4. Машины слитно быстро, цифры через многоточие
...
ЗАПРЕЩЕНО: цифры числами, "да?" в конце, "м-м", "ну-у", точки между фразами, мужской род, давить после отказа.
...
def process_audio_quality(mp3_bytes: bytes) -> bytes:
    try:
        ...
        if audio.channels == 2:
            noise_seg = noise_seg.set_channels(2)
