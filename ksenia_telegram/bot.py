"""
Ксения — Telegram демо-бот
Голосовой диалог: голосовое сообщение → STT → Claude → TTS → голосовое сообщение
"""

import asyncio
import logging
import aiohttp
import tempfile
import os
from telegram import Update, Voice
from telegram.ext import Application, CommandHandler, MessageHandler, filters, ContextTypes

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

# Конфигурация
TELEGRAM_TOKEN = os.environ.get("TELEGRAM_TOKEN", "")
YANDEX_API_KEY = os.environ.get("YANDEX_API_KEY", "")
YANDEX_FOLDER_ID = os.environ.get("YANDEX_FOLDER_ID", "")
OPENROUTER_API_KEY = os.environ.get("OPENROUTER_API_KEY", "")
ELEVENLABS_API_KEY = os.environ.get("ELEVENLABS_API_KEY", "")
ELEVENLABS_VOICE_ID = os.environ.get("ELEVENLABS_VOICE_ID", "")

# История диалогов по пользователям
conversations: dict = {}

SYSTEM_PROMPT = """Ты — Ксения, менеджер таксопарка Моментум. Звонишь бывшим водителям чтобы вернуть их в парк.

Твои принципы:
- Говоришь как живой человек, тепло и по-дружески
- НЕ давишь и не навязываешь
- Сначала выясняешь почему ушёл, потом предлагаешь решение
- Знаешь реальные цены: от 1790 р/день (Tiggo 4 Pro), первый день бесплатно
- Если водитель из другого парка — 3 дня бесплатно

Отвечай коротко — 1-2 предложения максимум. Это телефонный разговор."""


async def recognize_speech(audio_bytes: bytes, format: str = "oga") -> str:
    """Yandex SpeechKit STT"""
    url = "https://stt.api.cloud.yandex.net/speech/v1/stt:recognize"
    params = {
        "folderId": YANDEX_FOLDER_ID,
        "lang": "ru-RU",
        "format": format,
    }
    headers = {"Authorization": f"Api-Key {YANDEX_API_KEY}"}
    
    async with aiohttp.ClientSession() as session:
        async with session.post(url, params=params, headers=headers, data=audio_bytes) as resp:
            if resp.status == 200:
                data = await resp.json()
                return data.get("result", "")
            else:
                text = await resp.text()
                logger.error(f"STT error {resp.status}: {text}")
                return ""


async def generate_response(user_text: str, history: list) -> str:
    """Claude через OpenRouter"""
    history.append({"role": "user", "content": user_text})
    
    url = "https://openrouter.ai/api/v1/chat/completions"
    headers = {
        "Authorization": f"Bearer {OPENROUTER_API_KEY}",
        "Content-Type": "application/json"
    }
    payload = {
        "model": "anthropic/claude-sonnet-4-5",
        "messages": [
            {"role": "system", "content": SYSTEM_PROMPT},
            *history
        ],
        "max_tokens": 200,
        "temperature": 0.8
    }
    
    async with aiohttp.ClientSession() as session:
        async with session.post(url, json=payload, headers=headers) as resp:
            if resp.status == 200:
                data = await resp.json()
                reply = data["choices"][0]["message"]["content"]
                history.append({"role": "assistant", "content": reply})
                return reply
            else:
                return "Простите, что-то пошло не так. Можете повторить?"


async def synthesize_speech(text: str) -> bytes:
    """ElevenLabs TTS"""
    url = f"https://api.elevenlabs.io/v1/text-to-speech/{ELEVENLABS_VOICE_ID}"
    headers = {
        "xi-api-key": ELEVENLABS_API_KEY,
        "Content-Type": "application/json"
    }
    payload = {
        "text": text,
        "model_id": "eleven_multilingual_v2",
        "voice_settings": {
            "stability": 0.5,
            "similarity_boost": 0.75,
            "style": 0.3,
            "use_speaker_boost": True
        }
    }
    
    async with aiohttp.ClientSession() as session:
        async with session.post(url, json=payload, headers=headers) as resp:
            if resp.status == 200:
                return await resp.read()
            else:
                text_err = await resp.text()
                logger.error(f"TTS error {resp.status}: {text_err}")
                return b""


async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Начало диалога"""
    user_id = update.effective_user.id
    conversations[user_id] = []
    
    # Первая реплика Ксении голосом
    first_message = "Алло, добрый день! Это Ксения из таксопарка Моментум. Удобно пару минут поговорить?"
    
    await update.message.reply_text(f"🎙 *Ксения:* {first_message}", parse_mode="Markdown")
    
    # Синтезируем голос
    audio = await synthesize_speech(first_message)
    if audio:
        with tempfile.NamedTemporaryFile(suffix=".mp3", delete=False) as f:
            f.write(audio)
            f.flush()
            with open(f.name, "rb") as audio_file:
                await update.message.reply_voice(audio_file)
        os.unlink(f.name)
    
    await update.message.reply_text("🎤 Отправьте голосовое сообщение как водитель или напишите текстом")
    
    # Добавляем первую реплику в историю
    conversations[user_id].append({"role": "assistant", "content": first_message})


async def handle_voice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обработка голосового сообщения"""
    user_id = update.effective_user.id
    if user_id not in conversations:
        conversations[user_id] = []
    
    await update.message.reply_text("🔄 Слушаю...")
    
    # Скачиваем аудио
    voice = update.message.voice
    file = await context.bot.get_file(voice.file_id)
    audio_bytes = await file.download_as_bytearray()
    
    # STT
    user_text = await recognize_speech(bytes(audio_bytes), format="oga")
    if not user_text:
        await update.message.reply_text("❌ Не смогла разобрать. Попробуйте ещё раз или напишите текстом.")
        return
    
    await update.message.reply_text(f"👤 *Вы:* {user_text}", parse_mode="Markdown")
    
    # Claude
    reply = await generate_response(user_text, conversations[user_id])
    await update.message.reply_text(f"🎙 *Ксения:* {reply}", parse_mode="Markdown")
    
    # TTS
    audio = await synthesize_speech(reply)
    if audio:
        with tempfile.NamedTemporaryFile(suffix=".mp3", delete=False) as f:
            f.write(audio)
            f.flush()
            with open(f.name, "rb") as audio_file:
                await update.message.reply_voice(audio_file)
        os.unlink(f.name)


async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обработка текстового сообщения"""
    user_id = update.effective_user.id
    if user_id not in conversations:
        conversations[user_id] = []
    
    user_text = update.message.text
    
    # Claude
    reply = await generate_response(user_text, conversations[user_id])
    await update.message.reply_text(f"🎙 *Ксения:* {reply}", parse_mode="Markdown")
    
    # TTS
    audio = await synthesize_speech(reply)
    if audio:
        with tempfile.NamedTemporaryFile(suffix=".mp3", delete=False) as f:
            f.write(audio)
            f.flush()
            with open(f.name, "rb") as audio_file:
                await update.message.reply_voice(audio_file)
        os.unlink(f.name)


async def reset(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Сброс диалога"""
    user_id = update.effective_user.id
    conversations[user_id] = []
    await update.message.reply_text("🔄 Диалог сброшен. Напишите /start чтобы начать заново.")


def main():
    app = Application.builder().token(TELEGRAM_TOKEN).build()
    app.add_handler(CommandHandler("start", start))
    app.add_handler(CommandHandler("reset", reset))
    app.add_handler(MessageHandler(filters.VOICE, handle_voice))
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_text))
    
    logger.info("Ксения Telegram бот запущен!")
    app.run_polling(drop_pending_updates=True)


if __name__ == "__main__":
    main()
